  ...
  useEffect(() => {
    async function load() {
      try {
        ...
        // Load today's plan
        const planSnap = await getDoc(doc(db, 'dayPlans', today))
        if (planSnap.exists()) {
          setTodayPlan(planSnap.data())
        }
      } catch (err) {
        console.error(err)
      } finally {
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        ...
        <Link to="/counselor/schedule" className="card hover:shadow-md transition-shadow flex items-center gap-4 group">
          ...
          <div className="flex-1">
            ...
            <div className="text-sm text-gray-500">Планы за предыдущие дни</div>
          </div>
      ...
      <div className="card">
        <div className="flex items-center justify-between mb-4">
          ...
          {todayPlan?.publishedAt && (
            <span className="badge-green">Опубликован</span>
          )}
        </div>
        ...
        {loading ? (
          ...
          <div className="space-y-2">
            {allBlocks.map((block, i) => (
              <div key={block.id || i} className="flex items-start gap-3 p-3 rounded-lg hover:bg-gray-50 transition-colors">
                ...
                {block.isFixed && (
                  <span className="ml-auto badge-gray text-xs flex-shrink-0">постоянно</span>
                )}
              </div>
            ))}
          </div>
