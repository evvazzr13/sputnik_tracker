  ...
  useEffect(() => {
    async function load() {
      try {
        ...
        const planSnap = await getDocs(query(collection(db, 'dayPlans'), orderBy('date', 'desc')))
        const list = planSnap.docs
              ...
              >
                <div className="flex items-center gap-3">
                  ...
                  <div className="text-left">
                    ...
                    {isToday && <span className="ml-2 badge-blue">Сегодня</span>}
                  </div>
                </div>
                  ...
                  <div className="pt-3 space-y-1.5">
                    {blocks.map((block, i) => (
                      <div key={block.id || i} className="flex items-start gap-3 py-1.5">
                        <span className="text-sm font-mono text-blue-700 font-semibold w-12 flex-shrink-0">
                          {block.time}
                        </span>
